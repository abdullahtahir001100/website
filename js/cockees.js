...
// --- USER AUTHENTICATION / HEADER LOGIC ---
 ...
 * FIX: Added check for AUTH_CONTAINER existence.
 */
function renderAuthStatus() {
    // *** FIX IS HERE ***
    if (!AUTH_CONTAINER) {
        // If the container doesn't exist (e.g., on a page without a header), exit gracefully.
        console.warn('AUTH_CONTAINER (id="auth-container") not found in the DOM. Skipping header rendering.');
        return;
    }
    // *******************
    ...
    if (userData && userData.isLoggedIn) {
        ...
        const avatarHtml = `<div class="user-avatar" style="background-color: ${avatarColor};" onclick="handleAuthClick(event)">
            ${firstLetter}
        </div>
        ...
        const menuHtml = `<div id="user-menu-popup" class="user-menu-popup" style="display: none;">
        ...
        AUTH_CONTAINER.innerHTML = `<a href="${LOGIN_PAGE_URL}" aria-label="Login or Register">
